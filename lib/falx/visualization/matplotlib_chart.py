...
from lib.falx.visualization.visual_trace import BarV, BarH, Point, Line, Area, Box
from lib.falx.visualization.chart import VisDesign, remove_unused_fields
...
class MpAreaChart(object):
    ...
    def to_string_spec(self, mapping):
        cols = []
        for v in [self.c_x, self.c_tops, self.c_bots]:
            if v != None and not isinstance(v, (list,)):
                cols.append(mapping[v])
            elif isinstance(v, (list,)):
                cols += [mapping[v2] for v2 in v] 
