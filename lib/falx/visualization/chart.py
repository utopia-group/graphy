...
from lib.falx.symbolic import SymTable, SymVal
from lib.falx.utils import table_utils

from lib.falx.visualization import visual_trace
from lib.falx.visualization.visual_trace import BarV, BarH, Point, Line, Area, Box, Bar


def get_aggregate_func(func_name) -> Callable:
    if func_name == "mean":
    ...
    print("{} not supported".format(func_name))
    return None
...
def check_dimension_and_aggregate(chart, bar=False):
    single_dimension = False
    if "y" not in chart.encodings and "x" in chart.encodings:
        # aggregate_op
        aggregate_op = chart.encodings["x"].aggregate
        single_dimension = True
    else:
        # need to consider count operations here
        if bar:
            if chart.orientation == "horizontal":
                aggregate_op = chart.encodings["x"].aggregate
            else:
                aggregate_op = chart.encodings["y"].aggregate
        else:
            aggregate_op = chart.encodings["y"].aggregate
        # print(chart.encodings["x"].field)
        # print(chart.encodings["y"].field)
        if aggregate_op == "count" and (not chart.encodings["x"].field == chart.encodings["y"].field and
                                            (chart.encodings["x"].field is not None and
                                            chart.encodings["y"].field is not None)):
            print("wrong histogram format")
            raise Exception
    return single_dimension, aggregate_op


class VisDesign(object):
    """Top level visualization construct """

    def __init__(self, data, chart, transform=None):
    ...
    def to_vl_obj(self):
        ...
        if self.transform is not None:
            chart_obj["transform"] = self.transform
    ...
    def eval(self, ignore_orientation=False):
        return self.chart.eval(self.data, ignore_orientation)
    ...
    def load_from_vegalite(vl_spec, input_data):
        ...
        def check_column_type(col):
            return all(isinstance(item, int) for item in col if item is not None)

        # first deal with table level transformation
        transform = vl_spec["transform"] if ("transform" in vl_spec and len(vl_spec["transform"]) > 0) else None
        if transform is not None and type(transform[0]["filter"]) == str:
            input_data = table_utils.filter_table(input_data, transform[0]["filter"])
        else:
            input_data = table_utils.apply_transform(input_data, transform)
        ...
        if "layer" in vl_spec:
            ...
            for lspec in vl_spec["layer"]:
                ...
                if ("transform" in lspec and len(lspec["transform"]) > 0) and type(
                        lspec["transform"][0]["filter"]) == str:
                    pred = lspec["transform"][0]["filter"] if "transform" in lspec else "True"
                    data.append(table_utils.filter_table(input_data, pred))
                else:
                    ltransform = vl_spec["transform"] if (
                                "transform" in lspec and len(lspec["transform"]) > 0) else None
                    data.append(table_utils.apply_transform(input_data, ltransform))
        else:
            data = input_data
            layer_specs = [vl_spec]
        ...
        for lspec in layer_specs:
            ...
            encodings = [Encoding(channel, get_value(enc, "field"), get_value(enc, "type"), get_value(enc, "sort"),
                                  get_value(enc, "aggregate"), get_value(enc, "bin"))
                         for channel, enc in lspec["encoding"].items()]
            # print(encodings)
            # print(mark_ty)
            if mark_ty == "bar":
                orientation = "vertical"
                if "x" in lspec["encoding"] and "y" not in lspec["encoding"]:
                    orientation = "vertical"
                elif lspec["encoding"]["y"]["type"] == "nominal" and (
                        "aggregate" not in lspec["encoding"]["y"] or lspec["encoding"]["y"]["aggregate"] is None):
                    orientation = "horizontal"
                elif (lspec["encoding"]["y"]["type"] == "quantitative" \
                      and check_column_type([r[lspec["encoding"]["y"]["field"]] for r in data]) and (
                              "aggregate" not in lspec["encoding"]["y"] or lspec["encoding"]["y"][
                          "aggregate"] is None)):
                    orientation = "horizontal"
            ...
            elif mark_ty in ["point", "circle", "text", "rect", "tick"]:
        ...
        if len(layers) == 1:
            return VisDesign(data, layers[0], transform)
...
class BarChart(object):
    ...
    def to_ggplot2(self, data_var, alpha=1):
        ...
        return "{}(data={},aes({}),stat ='identity',alpha={}) + scale_x_discrete(){}{}".format(mark, data_var, aes_str,
                                                                                               alpha, coord_flip, facet)

    def eval(self, data, ignore_orientation):
        res = []
        ...
        single_dimension, aggregate_op = check_dimension_and_aggregate(self, bar=True)

        if aggregate_op == "count" and not self.encodings["x"].field == self.encodings["y"].field:
            print("wrong histogram format")
            raise Exception

        for key in grouped_data:
            vals = grouped_data[key]
            order_field = self.encodings["order"].field if self.encodings.get("order") is not None else None
            sort_order = self.encodings["order"].sort_order if self.encodings.get("order") is not None else None
            if sort_order != None:
                vals.sort(key=lambda x: x[order_field], reverse=True if sort_order == "descending" else False)

            color, _, column = key

            if aggregate_op is not None and not aggregate_op == "count":

                if single_dimension:
                    computed_value = get_aggregate_func(aggregate_op)([r[self.encodings["x"].field] for r in vals])
                    res.append(Bar(x=computed_value, y1=0, color=color, column=column))
                else:

                    if self.orientation == "horizontal":
                        # assume group by x-axis
                        group_by_field = self.encodings["y"].field
                    else:
                        group_by_field = self.encodings["x"].field
                    group_keys = set(r[group_by_field] for r in vals)
                    aggregate_group_by_data = {key: [r for r in vals if r[group_by_field] == key] for key in group_keys}

                    # assumption: if aggregation can be applied then this graph doesn't have y2/x2
                    for key in aggregate_group_by_data:
                        aggregate_vals = aggregate_group_by_data[key]
                        if self.orientation == "horizontal":
                            computed_value = get_aggregate_func(aggregate_op)(
                                [r[self.encodings["x"].field] for r in aggregate_vals])
                        else:
                            computed_value = get_aggregate_func(aggregate_op)(
                                [r[self.encodings["y"].field] for r in aggregate_vals])
                        if ignore_orientation:
                            res.append(Bar(x=key, y1=computed_value, color=color, column=column))
                        else:
                            if self.orientation == "horizontal":
                                res.append(BarH(x1=computed_value, y=key, color=color, column=column))
                            elif self.orientation == "vertical":
                                res.append(BarV(x=key, y1=computed_value, color=color, column=column))
            else:
                for r in vals:
                    if single_dimension:
                        v = get_channel_value(self.encodings, "x", r)
                        res.append(Bar(x=v, y1=0, y2=None, color=color, column=column))
                    else:
                        if ignore_orientation:
                            if self.orientation == "horizontal":
                                ...
                                res.append(Bar(x=y, y1=x1, y2=x2, color=color, column=column))
                            elif self.orientation == "vertical":
                                ...
                                res.append(Bar(x=x, y1=y1, y2=y2, color=color, column=column))
                        else:
                            if self.orientation == "horizontal":
                                x1 = get_channel_value(self.encodings, "x", r)
                                x2 = get_channel_value(self.encodings, "x2", r)
    ...
    def inv_eval(vtrace, orientation):
        ...
        if orientation == "vertical":
            for vt in vtrace:
                data_values.append(
                    {"c_x": vt.x, "c_y": vt.y1, "c_y2": vt.y2, "c_column": vt.column, "c_color": vt.color})
            channel_types = [("x", "nominal"), ("y", "quantitative"), ("y2", "quantitative"), ("color", "nominal"),
                             ("column", "nominal")]

        if orientation == "horizontal":
            for vt in vtrace:
                data_values.append(
                    {"c_x": vt.x1, "c_x2": vt.x2, "c_y": vt.y, "c_column": vt.column, "c_color": vt.color})
            channel_types = [("x", "quantitative"), ("x2", "quantitative"), ("y", "nominal"), ("color", "nominal"),
                             ("column", "nominal")]
...
class BoxPlot(object):
    ...
    def eval(self, data, ignore_orientation):
        ...
        if "y" not in self.encodings:
            ...
            get_group_key = lambda r: (r[self.encodings["y"].field] if "y" in self.encodings else None,
            ...
            for key in grouped_data:
                y, color, column = key
                ys = [r[self.encodings["x"].field] for r in grouped_data[key]]
                median = float(np.median(ys))
                upper_quartile = float(np.percentile(ys, 75))
                lower_quartile = float(np.percentile(ys, 25))
                res.append(Box(x=y, median=median, Q1=lower_quartile, Q3=upper_quartile,
    ...
    def inv_eval(vtrace):
        ...
        for vt in vtrace:
            ...
            constraints.append(
                "min([r.c_y for r in T if r.c_color == {} and r.c_column == {}]) = {}".format(vt.color, vt.column,
                                                                                              vt.min))
            constraints.append(
                "max([r.c_y for r in T if r.c_color == {} and r.c_column == {}]) = {}".format(vt.color, vt.column,
                                                                                              vt.max))
            constraints.append(
                "Q1([r.c_y for r in T if r.c_color == {} and r.c_column == {}]) = {}".format(vt.color, vt.column,
                                                                                             vt.Q1))
            constraints.append(
                "Q3([r.c_y for r in T if r.c_color == {} and r.c_column == {}]) = {}".format(vt.color, vt.column,
                                                                                             vt.Q3))
            constraints.append(
                "median([r.c_y for r in T if r.c_color == {} and r.c_column == {}]) = {}".format(vt.color, vt.column,
                                                                                                 vt.median))
...
class AreaChart(object):
    ...
    def eval(self, data, ignore_orientation):
        ...
        # need to consider count operations here
        aggregate_op = self.encodings["y"].aggregate
        if aggregate_op == "count" and not self.encodings["x"].field == self.encodings["y"].field:
            print("wrong histogram format")
            raise Exception
        ...
        for key in grouped_data:
            ...
            if aggregate_op is not None and not aggregate_op == "count":
                ...
                for key in aggregate_group_by_data:
                    ...
                    computed_value = get_aggregate_func(aggregate_op)(
                        [r[self.encodings["y"].field] for r in aggregate_vals])
                    res.append(Point(x=key, y=computed_value, color=color, column=column, presentation="area"))
            else:
                for val in vals:
                    res.append(Point(x=val[self.encodings["x"].field], y=val[self.encodings["y"].field], color=color,
                                     column=column, presentation="area"))
    ...
    def inv_eval(vtrace):
        ...
        for vt in vtrace:
            ...
            p1 = json.dumps({"c_x": vt.x1, "c_y": vt.yt1, "c_y2": vt.yb1, "c_color": vt.color, "c_column": vt.column},
                            sort_keys=True)
            p2 = json.dumps({"c_x": vt.x2, "c_y": vt.yt2, "c_y2": vt.yb2, "c_color": vt.color, "c_column": vt.column},
                            sort_keys=True)
        ...
        channel_types = [("x", "_"), ("y", "quantitative"), ("y2", "quantitative"), ("color", "nominal"),
                         ("column", "nominal")]
...
class LineChart(object):
    ...
    def __init__(self, encodings):
        ...
        if "order" not in self.encodings:
            ...
            if self.encodings["x"].enc_ty == "temporal":
                # self.encodings["order"] = Encoding(channel="order", field=self.encodings["x"].field, enc_ty="temporal")
                pass
            else:
                self.encodings["order"] = Encoding(channel="order", field=self.encodings["x"].field,
                                                   enc_ty="quantitative")
    ...
    def eval(self, data, ignore_orientation):
        ...
        # need to consider count operations here
        single_dimension, aggregate_op = check_dimension_and_aggregate(self)
        ...
        # print("group data: " + str(grouped_data))
        ...
        for key in grouped_data:
            ...
            if aggregate_op is not None and not aggregate_op == "count":
                ...
                if single_dimension:
                    computed_value = get_aggregate_func(aggregate_op)([r[self.encodings["x"].field] for r in vals])
                    res.append(Point(x=computed_value, y=0, size=size, color=color, column=column, presentation="line"))
                else:
                    # assume group by x-axis
                    group_by_field = self.encodings["x"].field
                    # group_by_field_type = self.encodings["x"].type
                    group_keys = set(r[group_by_field] for r in vals)
                    aggregate_group_by_data = {key: [r for r in vals if r[group_by_field] == key] for key in group_keys}
                    ...
                    for key in aggregate_group_by_data:
                        ...
                        computed_value = get_aggregate_func(aggregate_op)(
                            [r[self.encodings["y"].field] for r in aggregate_vals])
                        res.append(
                            Point(x=key, y=computed_value, size=size, color=color, column=column, presentation="line"))
            else:
                for val in vals:
                    if single_dimension:
                        res.append(Point(x=val[self.encodings["x"].field], y=0, size=size, color=color, column=column,
                                         presentation="line"))
                    else:
                        res.append(Point(x=val[self.encodings["x"].field], y=val[self.encodings["y"].field], size=size,
                                         color=color, column=column, presentation="line"))
                # for i in range(len(vals) - 1):
                #     l, r = vals[i], vals[i + 1]
    ...
    def inv_eval(vtrace):
        ...
        for vt in vtrace:
            # each end of an point will only be added once
            p1 = json.dumps({"c_x": vt.x1, "c_y": vt.y1, "c_size": vt.size, "c_color": vt.color, "c_column": vt.column},
                            sort_keys=True)
            p2 = json.dumps({"c_x": vt.x2, "c_y": vt.y2, "c_size": vt.size, "c_color": vt.color, "c_column": vt.column},
                            sort_keys=True)
            if p1 not in frozen_data: frozen_data.append(p1)
            if p2 not in frozen_data: frozen_data.append(p2)
...
class ScatterPlot(object):
    ...
    def eval(self, data, ignore_orientation):
        ...
        res = []
        # handle single-dimension plot
        single_dimension, aggregate_op = check_dimension_and_aggregate(self)

        if ("color" in self.encodings and self.encodings["color"].field is None) or     \
            ("size" in self.encodings and self.encodings["size"].field is None) or  \
            ("column" in self.encodings and self.encodings["column"].field is None) :
            raise SyntaxError
        ...
        for key in grouped_data:
            ...
            if aggregate_op is not None and not aggregate_op == "count":
                ...
                if single_dimension:
                    computed_value = get_aggregate_func(aggregate_op)([r[self.encodings["x"].field] for r in vals])
                    res.append(Point(x=computed_value, y=0, size=size, color=color, column=column, presentation="tick"))
                    ...
                    for key in aggregate_group_by_data:
                        ...
                        computed_value = get_aggregate_func(aggregate_op)(
                            [r[self.encodings["y"].field] for r in aggregate_vals])
                        res.append(
                            Point(x=key, y=computed_value, size=size, color=color, column=column, presentation="point"))
            else:
                for val in vals:
                    if single_dimension:
                        assert not aggregate_op == "count"
                        res.append(Point(x=val[self.encodings["x"].field], y=0, size=size, color=color, column=column,
                                         presentation="tick"))
                    else:
                        res.append(Point(x=val[self.encodings["x"].field], y=val[self.encodings["y"].field], size=size,
                                         color=color, column=column, presentation="point"))
        # for r in data:
        #     x = get_channel_value(self.encodings, "x", r)
        #     y = get_channel_value(self.encodings, "y", r)
        #     color = get_channel_value(self.encodings, "color", r)
...
class Encoding(object):
    def __init__(self, channel, field, enc_ty, sort_order=None, aggregate=None, bin=None):
        """ sort order is either descending/ascending or default (unsorted)"""
        self.channel = channel
        self.field = field
        self.enc_ty = enc_ty
        self.sort_order = sort_order
        self.aggregate = aggregate if aggregate is not None and not aggregate == "null" else None
        self.bin = bin
