...
from lib.node import VariableNode, TerminalNode
from lib.program import Program
from lib.synthesizer.program_synthesizer import ProgramSynthesizer
from lib.type.base_type import TableType, NullType, ConstColType, NullOpType
from lib.type.predicate import Predicate, Prov, RelationPred, Constant
...
from lib.utils.synth_utils import get_curr_enc_assignment


class ProgramSynthesizerNoQualifier(ProgramSynthesizer):
    def __init__(self, grammar: TypedCFG, no_provenance=False, no_table=False, base_type=True):
        super().__init__(grammar, no_prov=no_provenance, no_table=no_table)
    ...
    def synthesize(self, spec: BaseRefType, _input: InputDataset) -> List[Program]:
        ...
        # print("num curr progs: ", len(curr_progs))
        # print("finished synthesizing visualization")
        # print("num curr progs: ", len(curr_progs))

        for prog in curr_progs:
            # print('curr_prog:', repr(prog))
            # print("preupdated goal type:", prog.get_goal_type(prog.table_root_node))

            try:
                prog.update_goal_type(no_prov=self.no_provenance)
            except TypeError:
                print("type error, continue to the next goal")
                continue

            # print("postupdated goal type:", prog.get_goal_type(prog.table_root_node))
            ...
            if not self.no_provenance and format_key in self.explored_progs_types:
                continue
            else:
                self.explored_progs_types.append(format_key)

            # print("updated goal type:", prog.get_goal_type(prog.table_root_node))

            self.pruned_productions = []
            synthesized_prog += self.synthesize_table_transformation_top_down(prog, _input)

        # print("main synthesized_prog: ", synthesized_prog)

        return synthesized_prog

    def enumerate_strengthen_prog(self, prog: Program, input_type=None) -> List[Program]:
        """
        ...
        # print("enumerate_strengthen_prog:", prog)

        strengthen_progs: List[Program] = []
        plot_func_nid: int
        strengthened_types: List[RefType]
        plot_func_nid, strengthened_types = prog.viz_table_input_strengthening_2(True, input_type=input_type, no_prov=self.no_provenance)

        # print("strengthened types: ", strengthened_types)
    ...
    def synthesize_table_transformation_top_down(self, curr_prog: Program, input: InputDataset) -> List[Program]:
        ...
        # printd("curr_prog.table_root_node.goal_type:", curr_prog.get_goal_type(curr_prog.table_root_node))
            ...
            if curr_candidate.is_concrete('table'):
                ...
                if len(curr_candidate.nodes_pushed) > 1:
                    while len(curr_candidate.nodes_pushed) > 1:
                        ...
                        curr_candidate.update_type(input.input_type, curr_node_id, has_input_type=not self.no_table, no_prov=self.no_provenance)
                ...
                # print("concrete prog:", curr_candidate)
                ...
                if subtyping_res.error_code == 1:
                    ...
                    # print("synthesized_program:", synthesized_program)
                else:

                    printd("failed subtyping check")
                    # print(subtyping_res.error_code)
                continue
    ...
    def expand_nonterminal_sym_table_transformation(self, curr_prog: Program, selected_var: VariableNode, input_type: BaseRefType) -> List[Program]:
        ...
        # print("goal_type:", curr_prog.get_goal_type(selected_var))
        # print("goal_type_fieds:", curr_prog.get_goal_type(selected_var).base.fields)
        ...
        # print("applicable_productions:", applicable_productions)
        ...
        for prod, pred in applicable_productions:
            ...
            if prod.function_name is 'select':
                ...
                complete_prog.update_type(input_type, selected_var.id, has_input_type=not self.no_table, no_prov=self.no_provenance)  # TODO: need to cache forward computation results
                ...
                if prod.function_name is 'summarize':
                    ...
                    prog_var_node_children[0].set_goal_type(prod.inverse(prog_new.get_goal_type(selected_var), 0, input_type, self.no_provenance, op, col))
    ...
    def init_base_case_prog(self, curr_prog: Program, input: InputDataset) -> Tuple[List[Program], List[Program]]:
        ...
        compatiblity_check = check_compatibility(base_case_program.type, curr_prog.get_goal_type(curr_prog.table_root_node))
        if compatiblity_check.error_code == 1:
            printd("pass subtyping check")
    ...
    def expand_nonterminal_sym(self, curr_prog: Program, var_node: VariableNode, input_type: Optional[BaseRefType] = None) -> List[Program]:

        assert isinstance(var_node.sym, NonterminalSymbol)

        new_progs: List[Program] = []

        # first, according to the symbol, find productions with the appropriate goal type
        applicable_productions = self.grammar.select_productions(var_node, curr_prog.get_goal_type(var_node))
        ...
        for prod in applicable_productions:
            ...
            prog_new: Program = curr_prog.duplicate(next(self.program_counter))
            prod.apply(prog_new, var_node, True, input_type, no_prov=self.no_provenance)
            new_progs.append(prog_new)
    ...
    def expand_terminal_sym_with_value(self, curr_prog: Program, selected_var: VariableNode, value, duplicate=True) -> Program:
        ...
        var_goal_type = curr_prog.get_goal_type(selected_var).duplicate_ignore_base()
    ...
    def expand_terminal_sym(self, curr_prog: Program, selected_var: VariableNode, input: InputDataset = None, add_none=False) -> List[Program]:
        ...
        # print("curr_prog_exapnadf_terminal_sym:", curr_prog)
        ...
        elif isinstance(var_goal_type.base, NullOpType):
            if self.no_provenance:

                # first figure out which argument this is
                if var_goal_type.enc == 'color' or var_goal_type.enc == 'column':
                    # color and column are both optional
                    new_progs.append(self.expand_terminal_sym_with_value(curr_prog, selected_var, 'None'))

                # print("var_goal_type_enc:", var_goal_type.enc)
                known_enc_column_assignment = curr_prog.get_known_enc_col_assignment(curr_prog.get_parent(selected_var.id))
                # print("known_enc_col_assignment:", known_enc_column_assignment)
                to_be_assigned_values = get_curr_enc_assignment(self.grammar.name_to_sym['sym_col'].values, known_enc_column_assignment,
                                                                color_or_column=True if (var_goal_type.enc == 'color' or var_goal_type.enc == 'column') else False)
                # print('to be assigned:', to_be_assigned_values)

                for field in to_be_assigned_values:
                    new_progs.append(self.expand_terminal_sym_with_value(curr_prog, selected_var, field))
        else:
