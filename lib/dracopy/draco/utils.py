...
def asp_to_vl(facts):
    ...
    for e in encodings:
        ...
        if "aggregate" in enc:
            ...
            if enc["aggregate"] == 'count' and enc["channel"] == "x" or enc["channel"] == "y":
                exist_count = True
        if "field" in enc:
        ...
        if "hide_axis" in enc:
            encoding[enc["channel"]]["axis"] = None

    if exist_count:
        if x_field is None and y_field is not None:
            assert encoding["x"]["aggregate"] == 'count'
            encoding["x"]["field"] = y_field
        elif y_field is None and x_field is not None:
            assert encoding["y"]["aggregate"] == 'count'
            encoding["y"]["field"] = x_field
